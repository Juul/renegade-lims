...
module.exports = function(settings) {
  ...
  const rimbaudAPI = {
    ...
    synchronizeOrders: function(labCore, feed, cb) {
      ...
      })).on('end', function(didAnyFail) {
        if(cb) cb();
      });
    },

    repeatSync: function(labCore) {
      rimbaudAPI.synchronizeOrders(labCore, null, function(didAnyFail) {
        setTimeout(function() {
          rimbaudAPI.repeatSync(labCore);
        }, settings.synchronizeOrders * 1000);
      });
    },
    ...
    postOrderAndRemember: function(labCore, key, sample, cb) {
      ...
      if(sample.rimbaudSynced) return cb();
      rimbaudAPI.postOrder(sample, function(err) {
        if(err) return cb(err);
        
        labCore.api.swabTubesByTimestamp.markAsRimbaudSynced(key, sample, cb);
      });
    },
