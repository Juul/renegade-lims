...
const writer = {
  ...
  saveQpcrResult(labCore, result, cb) {
    ...
    labCore.writer('results', function(err, feed) {
      ...
      if(!resultID) {
        return append(feed, res, (err) => {
          cb(err, res.id);
        });
      }

      // If an ID was specified, verify that the result exists in the db
      // (we don't want the client arbitrarily deciding on IDs)
      labCore.api.objectsByGUID.get(resultID, function(err, oldResult) {
        if(err) return cb(err);
